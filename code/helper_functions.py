...
def jsonify(obj, out_file):
    ...
    """
    json.dump(obj, codecs.open(out_file, 'w', encoding='utf-8'),
...
def add_time_diff_per_aid_col(df, order_by_time=False):
    ...
    # get bool mask of to filter out first msg of each group
    mask = df.aid == df.aid.shift(1)
    df = df[mask]
    if order_by_time:
...
def get_injection_interval(df, injection_aid, injection_data_str, max_injection_t_delta=1):
    ...
    attack_messages_df = df[(df.aid == injection_aid) & (
        df.data.str.contains(injection_data_str))]  # get subset of attack messages
...
def add_kde_val_col(df, d):
    ...
    new_column = np.concatenate([d[aid]["kde"].evaluate(
        df[df.aid == aid].time_diffs.values) for aid in tqdm(df.aid.unique())])
...
def add_gauss_val_col(df, d):
    ...
    new_column = np.concatenate([d[aid]["gauss"].pdf(
        df[df.aid == aid].time_diffs.values) for aid in tqdm(df.aid.unique())])
...
def alert_by_mean(df, d):
    ...
    """
    df['predicted_attack'] = df.apply(
        lambda row: row.time_diffs <= (d[row.aid]['mu']/2), axis=1)
    return df
...
def alert_by_mean_various_p(df, d, p):
    ...
    """
    df['predicted_attack'] = df.apply(
        lambda row: row.time_diffs <= (p*d[row.aid]['mu']), axis=1)
    return df
...
def alert_by_kde(df, d, p):
    ...
    """
    df['predicted_attack'] = df.apply(
        lambda row: row.kde_val <= d[row.aid]['y_thresholds_kde'][p], axis=1)
    return df
...
def alert_by_gauss(df, d, p):
    ...
    """
    df['predicted_attack'] = df.apply(
        lambda row: row.gauss_val <= d[row.aid]['y_thresholds_gauss'][p], axis=1)
    return df
...
def signal_count_new(df, aid):
    ...
    try:
        ...
        breakpoints = [bisect.bisect_right(
            times_vec, i*4*mu) for i in range(int(max(times_vec)/(4*mu)))]
...
def alert_by_bin(df, d, n=6):
    ...
    for aid in df.aid.unique():
        ...
        df_test['predicted_attack'] = df_test.time_diffs.rolling(
            n).sum() <= d[aid]['mu']*4
        cm_aid = confusion_matrix(
            df_test['actual_attack'], df_test['predicted_attack'], labels=[0, 1])
        cm += cm_aid
        #print(aid, cm_aid)
    return cm
...
def get_results_mean(attack_list, d):
    ...
    for i in tqdm(range(len(attack_list))):
        ...
        attack_list[i]['alert_window'] = attack_list[i].predicted_attack.rolling(
            6).sum() >= 3
        cm = confusion_matrix(
            attack_list[i]['actual_attack'], attack_list[i]['alert_window'])
    ...
    results_mean['total']['prec'] = results_mean['total']['cm'][1, 1] / \
        (results_mean['total']['cm'][1, 1]+results_mean['total']['cm'][0, 1])
    results_mean['total']['recall'] = results_mean['total']['cm'][1, 1] / \
        (results_mean['total']['cm'][1, 1]+results_mean['total']['cm'][1, 0])
    results_mean['total']['f1'] = 2*((results_mean['total']['prec']*results_mean['total']['recall'])/(
        results_mean['total']['prec']+results_mean['total']['recall']))
    results_mean['total']['false_pos'] = results_mean['total']['cm'][0, 1] / \
        (results_mean['total']['cm'][0, 1]+results_mean['total']['cm'][0, 0])

    # print(os.path.dirname(os.getcwd()))

    # return(results_mean)

    picklify(results_mean, os.path.dirname(
        os.getcwd()) + "/results_mean_final.pkl")
...
def get_results_mean_various_p(attack_list, d):
    ...
    results_mean_final = {}
    for p in pvals_mean:
        results_mean_final[p] = {'cm': [0],
                                 'recall': 0, 'prec': 0, 'false_pos': 0}
    ...
    for p in tqdm(pvals_mean):
        ...
        results_mean_final[p]['prec'] = details[1, 1] / \
            (details[1, 1]+details[0, 1])
        results_mean_final[p]['recall'] = details[1, 1] / \
            (details[1, 1]+details[1, 0])
        results_mean_final[p]['f1'] = 2*((results_mean_final[p]['prec']*results_mean_final[p]['recall'])/(
            results_mean_final[p]['prec']+results_mean_final[p]['recall']))
        results_mean_final[p]['false_pos'] = details[0, 1] / \
            (details[0, 1] + details[0, 0])

    # print(os.path.dirname(os.getcwd()))

    # return(results_mean_final)

    picklify(results_mean_final, os.path.dirname(
        os.getcwd()) + "/results_mean_final.pkl")
...
def get_results_mean_various_p_no_pretraining(attack_list, d):
    ...
    picklify(results_mean_final, os.path.dirname(os.getcwd()) +
             "/results_mean_final_no_pretraining.pkl")
...
def get_results_kde(pvals, attack_list, d):
    ...
    pvals_kde = sorted(list(np.arange(0.001, 0.01, 0.001)) +
                       list(np.arange(0, 0.1, 0.01)))

    results_kde_final = {}
    for p in pvals_kde:
        results_kde_final[p] = {'cm': [0],
                                'recall': 0, 'prec': 0, 'false_pos': 0}
    ...
    for p in tqdm(pvals):
        ...
        results_kde_final[p]['recall'] = details[1, 1] / \
            (details[1, 1]+details[1, 0])
        results_kde_final[p]['prec'] = details[1, 1] / \
            (details[1, 1]+details[0, 1])
        ...
        results_kde_final[p]['false_pos'] = details[0, 1] / \
            (details[0, 1]+details[0, 0])
    picklify(results_kde_final, os.path.dirname(
        os.getcwd()) + "/results_kde_final.pkl")
...
def get_results_kde_no_pretraining(pvals, attack_list, d):
    ...
    picklify(results_kde_final, os.path.dirname(os.getcwd()) +
             "/results_kde_final_no_pretraining.pkl")
...
def get_results_gauss(attack_list, d):
    ...
    pvals_gauss = sorted(list(np.arange(0.001, 0.01, 0.001)
    ...
    for p in tqdm(pvals_gauss):
        ...
        results_gauss_final[p]['recall'] = details[1, 1] / \
            (details[1, 1]+details[1, 0])
        results_gauss_final[p]['prec'] = details[1, 1] / \
            (details[1, 1]+details[0, 1])
        results_gauss_final[p]['f1'] = 2*((results_gauss_final[p]['prec']*results_gauss_final[p]['recall'])/(
            results_gauss_final[p]['prec']+results_gauss_final[p]['recall']))
        results_gauss_final[p]['false_pos'] = details[0,
                                                      1]/(details[0, 1]+details[0, 0])

    picklify(results_gauss_final, os.path.dirname(
        os.getcwd()) + "/results_gauss_final.pkl")
...
def get_results_gauss_no_pretraining(attack_list, d):
    ...
    picklify(results_gauss_final, os.path.dirname(os.getcwd()) +
             "/results_gauss_final_no_pretraining.pkl")
...
def get_results_binning(attack_list, D, n=6):
    ...
    results_binning = {}
    for i in range(len(attack_list)):
        results_binning[i+1] = {'cm': [0],
                                'recall': 0, 'prec': 0, 'false_pos': 0}
    results_binning['total'] = {'cm': [0],
                                'recall': 0, 'prec': 0, 'false_pos': 0}
...
def get_results_binning_various_p(attack_list, D, n=6):
    ...
    for p in tqdm(pvals_binning):
        ...
        results_binning_final[p]['prec'] = details[1, 1] / \
            (details[1, 1]+details[0, 1])
        results_binning_final[p]['recall'] = details[1,
                                                     1]/(details[1, 1]+details[1, 0])
        results_binning_final[p]['f1'] = 2*((results_binning_final[p]['prec']*results_binning_final[p]['recall'])/(
            results_binning_final[p]['prec']+results_binning_final[p]['recall']))
        results_binning_final[p]['false_pos'] = details[0,
                                                        1]/(details[0, 1] + details[0, 0])

    # return results_binning_final

    picklify(results_binning_final, os.path.dirname(
...
def get_results_binning_various_p_no_pretraining(attack_list, D, n=6):
    ...
    picklify(results_binning_final, os.path.dirname(os.getcwd()) +
             "/results_binning_final_no_pretraining.pkl")
...
def get_results_binning_J1939(attack_list, D, n=6):
    ...
    for i in range(len(attack_list)):
        try:
            results_binning[i +
                            1]['cm'] = alert_by_bin_J1939(attack_list[i], D, n)
            results_binning[i+1]['prec'] = results_binning[i+1]['cm'][1, 1] / \
                (results_binning[i+1]['cm'][1, 1] +
                 results_binning[i+1]['cm'][0, 1])
            results_binning[i+1]['recall'] = results_binning[i+1]['cm'][1, 1] / \
                (results_binning[i+1]['cm'][1, 1] +
                 results_binning[i+1]['cm'][1, 0])
            results_binning[i+1]['false_pos'] = results_binning[i+1]['cm'][0, 1] / \
                (results_binning[i+1]['cm'][0, 1] +
                 results_binning[i+1]['cm'][0, 0])
            results_binning['total']['cm'] += results_binning[i+1]['cm']
        except:
            pass
    try:
